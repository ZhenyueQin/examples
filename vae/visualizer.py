...
def getReconstructedImages(X, autoencoder):
    ...
    for i in range(nbSamples):
        ...
        resultImage[rowIndex * imageSize:(rowIndex + 1) * imageSize,
        columnIndex * 2 * imageSize:(columnIndex + 1) * 2 * imageSize, :] = np.hstack([original, reconstruction])
...
def visualizeReconstructedImages(X_train, X_test, autoencoder, save=False, label=False):
    ...
    if save:
        cv2.imwrite('results/' + "reconstructions_{}.png".format(label), result)
    else:
...
def visualizeArithmetics(a, b, c, encoder, decoder):
    ...
    cv2.imshow("Arithmetics in latent space",
               np.hstack([reconstructedA, reconstructedB, reconstructedC, reconstructedAdd, reconstructedAddSub]))
    cv2.waitKey()
...
def visualizeInterpolation(start, end, encoder, decoder, save=False, nbSteps=5):
    ...
    for i in range(len(reconstructions)):
        ...
        if save:
            cv2.imwrite('results/' + "{}_{}.png".format(hashName, i),
