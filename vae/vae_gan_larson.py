...
G = VAE_GAN_Generator(input_channels, hidden_size).to(device)
D = Discriminator(input_channels).to(device)
...
fixed_noise = Variable(torch.randn(batch_size, hidden_size)).to(device)
data, _ = next(iter(train_loader))
